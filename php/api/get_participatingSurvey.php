...
$statement = $connection->prepare("SELECT SA.Sid, S.Title, S.End_Date
                                        FROM SurveysAvailable SA, Survey S 
                                            WHERE SA.email = ? AND SA.Sid = S.Sid");
...
if($count == 0){
    send_JSON_error("User doesnt have any surveys to participate in :(");
}
else {
