...
$statement = $connection->prepare("SELECT T2Q.T2id, T2Q.Question
                                    FROM SurveyProject.Survey S, SurveyProject.T2Questions T2Q
                                        WHERE S.Sid = ? AND S.Sid = T2Q.Sid");

$statement->bind_param("i", $in_data["Sid"]);
$statement->execute();
...
$statement = $connection->prepare("SELECT T1Q.T1id, T1Q.Question
                                    FROM SurveyProject.Survey S, SurveyProject.T1Questions T1Q
                                        WHERE S.Sid = ? AND S.Sid = T1Q.Sid");
...
if($count == 0){
    send_JSON_error("Survey has no questions :(");
}
else {
