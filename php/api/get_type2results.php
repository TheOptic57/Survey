...
$statement = $connection->prepare("SELECT T2Q.T2id, T2Q.Question, T2A.Answer
                                    FROM Survey S, T2Answers T2A, T2Questions T2Q
                                        WHERE S.Sid = ? AND S.Sid = T2Q.Sid AND T2Q.T2id = T2A.T2id
                                            ORDER BY T2Q.T2id;");
