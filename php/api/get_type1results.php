...
$statement = $connection->prepare("SELECT T1Q.T1id, T1Q.Question, T1A.Answer
                                    FROM Survey S, T1Answers T1A, T1Questions T1Q
                                        WHERE S.Sid = ? AND S.Sid = T1Q.Sid AND T1Q.T1id = T1A.T1id
                                            ORDER BY T1Q.T1id");
...
if($count == 0){
    send_JSON_error("Theres no Type 1 data for this survey!");
}
else {
